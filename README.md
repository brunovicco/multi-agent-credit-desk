...
## Current state: Milestone 1 — workspace foundation

This repository currently contains **scaffolding only**: a uv workspace with two empty, independently
testable packages. No agents, no orchestrator, no MCP servers, and no scoring logic are implemented
yet.

```text
multi-agent-credit-desk/
├── packages/
│   ├── contracts/      # import: credit_desk_contracts — shared schemas (scaffolded, empty)
